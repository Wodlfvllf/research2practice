    ...
    def forward(self, x):
        ...
        for i_level in range(self.num_resolutions):
            ...
            if len(self.down[i_level].attn) > 0:
                h = self.down[i_level].attn[0](h) # Assuming only one attention block per level if present
            if i_level != self.num_resolutions - 1:
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if len(self.up[i_level].attn) > 0:
                h = self.up[i_level].attn[0](h) # Assuming only one attention block per level if present
            if i_level != 0:
