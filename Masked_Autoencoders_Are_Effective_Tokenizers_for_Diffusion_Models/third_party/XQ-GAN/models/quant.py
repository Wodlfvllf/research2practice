...
import dist # this file only provides the VectorQuantizer2 used in VQVAE
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                h_BChw = F.interpolate(self.embedding(idx_Bhw).permute(0, 3, 1, 2), size=(H, W), mode='bicubic').contiguous() \
                    if (si != SN-1) else self.embedding(idx_Bhw).permute(0, 3, 1, 2).contiguous()
            ...
            margin = tdist.get_world_size() * (f_BChw.numel() / f_BChw.shape[1]) / self.vocab_size * 0.08 # margin = pn*pn / 100
