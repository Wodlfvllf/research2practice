...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, attn_bias):
        ...
        if using_flash:
            ...
            oup = memory_efficient_attention(q.to(dtype=main_type), k.to(dtype=main_type), v.to(dtype=main_type), attn_bias=None if attn_bias is None else attn_bias.to(
                dtype=main_type).expand(B, self.num_heads, -1, -1), p=dropout_p, scale=self.scale).view(B, -1, C)[:,:L,:]
        else:
...
class AdaLNSelfAttn(nn.Module):
    ...
    def forward(self, x, cond_BD, attn_bias):
        ...
        x = x + self.drop_path(self.attn(
            self.ln_wo_grad(x).mul(scale1.add(1)).add(shift1),
            attn_bias=attn_bias).mul(gamma1))
        x = x + self.drop_path(self.ffn(
            self.ln_wo_grad(x).mul(scale2.add(1)).add(shift2)).mul(gamma2)) # this mul(gamma2) cannot be in-placed when FusedMLP is used
        return x
